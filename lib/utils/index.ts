import { DateConfig, dateConfigMap, Theme, Themes, } from '../types'

export const isTouchEvent = <T extends HTMLElement>(e: TargetedTouchEvent<T> | TargetedMouseEvent<T>): e is TargetedTouchEvent<T> => {
...
export const isWheelEvent = <T extends HTMLElement>(e: TargetedTouchEvent<T> | TargetedMouseEvent<T> | TargetedWheelEvent<T>): e is TargetedWheelEvent<T> => {
    if (!('deltaY' in e)) {
        return false
    }
    return true
